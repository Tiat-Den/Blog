...
export async function generateStaticParams() {
  const viFiles = await getFiles("vi", "notes");
  const enFiles = await getFiles("en", "notes");
...
export async function generateMetadata(props: BrainPageProps): Promise<Metadata> {
  ...
  const { lang } = params;
  const note = await getFileContent(lang, "notes", `${params.slug}.mdx`, noteFrontmatterSchema);
  if (!note) return {};
...
export default async function BrainNotePage(props: BrainPageProps) {
  ...
  if (note.metadata.related && note.metadata.related.length > 0) {
    const allNotes = await getAllContent(lang, "notes", noteFrontmatterSchema);
    ...
    <Container className="py-12 flex flex-col gap-12 max-w-4xl">
      <header className="flex flex-col gap-4 border-b pb-8">
        <div className="flex items-center gap-2 text-muted-foreground text-sm mb-2">
          <Link href={`/${lang}/brain`} className="hover:text-foreground transition-colors">Brain</Link>
          <span>/</span>
        ...
        <section className="mt-12 pt-8 border-t">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {relatedNotes.map((relNote) => (
              <Link key={relNote.slug} href={`/${lang}/brain/${relNote.slug}`}>
