...
export async function generateMetadata(props: BlogPostPageProps): Promise<Metadata> {
  ...
  const post = await getFileContent("posts", `${params.slug}.mdx`, postFrontmatterSchema);
  if (!post) return {};
  ...
  return (
    <Container className="py-12 flex flex-col gap-12 max-w-3xl">
      <header className="flex flex-col gap-4">
        <Link href="/blog" className="text-sm font-mono text-muted-foreground hover:text-foreground transition-colors">
          ← Back to Blog
        </Link>
