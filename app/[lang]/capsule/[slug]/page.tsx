...
export async function generateStaticParams() {
  const viFiles = await getFiles("vi", "capsule");
  const enFiles = await getFiles("en", "capsule");
...
export async function generateMetadata(props: CapsulePageProps): Promise<Metadata> {
  ...
  const { lang } = params;
  const capsule = await getFileContent(lang, "capsule", `${params.slug}.mdx`, capsuleFrontmatterSchema);
  if (!capsule) return {};
...
export default async function CapsulePage(props: CapsulePageProps) {
  ...
  // Find prev/next
  const allCapsules = await getAllContent(lang, "capsule", capsuleFrontmatterSchema);
  ...
  return (
    <Container className="py-12 flex flex-col gap-12 max-w-3xl">
      <header className="flex flex-col gap-6 text-center border-b pb-8 border-dashed items-center">
        <Link href={`/${lang}/capsule`} className="text-sm font-mono text-muted-foreground hover:text-foreground transition-colors mb-4">
          ← Back to Vault
        </Link>
      ...
      <div className="flex justify-between items-center border-t border-dashed pt-8 mt-8">
        {prevCapsule ? (
          <Button variant="ghost" asChild className="flex flex-col items-start h-auto py-2">
            <Link href={`/${lang}/capsule/${prevCapsule.slug}`}>
        ...
        {nextCapsule ? (
          <Button variant="ghost" asChild className="flex flex-col items-end h-auto py-2">
            <Link href={`/${lang}/capsule/${nextCapsule.slug}`}>
